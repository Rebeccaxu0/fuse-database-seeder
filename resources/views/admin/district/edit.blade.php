<x-app-layout>
    ...
    <form class="mt-6" action="{{ route('admin.districts.update', $district) }}" method="POST">
        ...
        <div class="mb-4">
            <label class="text-gray-700 mb-4">{{ __('Current Super Facilitators') }}</label>
        ...
        <div class="mb-4">
          <label class="text-gray-700 mb-4">{{ __('Current Schools') }}</label>
          <p class="text-xs">{{ __('Mark for removal') }}</p>
        ...
        <div x-data="{active: true}">
            ...
            <span class="mx-2 text-xs text-gray-700">{{
              __('Unchecking this will deactivate all schools in the district and move all associated students/facilitators into the Alumni Studio.') }}</span><br>
