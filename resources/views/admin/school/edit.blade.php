<x-app-layout>
    ...
    <form action="{{ route('admin.schools.update', $school) }}" method="POST">
        ...
        <div x-data="{active: true}">
            ...
            <span
                class="mx-2 text-xs text-gray-700">{{                 class="mx-2 text-xs text-gray-700">__('Unchecking this will deactivate all studios in the school and move all associated students/facilitators into the Alumni Studio.') }}</span><br>
            ...
            <span class="mx-2 text-xs text-gray-700 "
                :class="{ 'text-gray-400': active}">{{                 :class="{ 'text-gray-400': active}">__('WARNING: This is a destructive operation and cannot be undone.') }}</span><br>
        </div>
        <div class="flex flex-wrap mt-4 -mx-3 mb-2">
            <button type="submit" id="btn-submit"
                class="text-md h-12 px-6 m-2 bg-fuse-green rounded-lg text-white">{{                 class="text-md h-12 px-6 m-2 bg-fuse-green rounded-lg text-white">__(' Update School') }}</button>
        </div>
    </form>
